...
public class DatabaseConfig {
    ...
    private void configureFromPassword(HikariConfig config) {
        // Parse username:password@hostname:port/database
        ...
        config.setJdbcUrl("jdbc:sqlserver://" + hostPort[0] + ":" + hostPort[1] + ";databaseName=" + hostPortDb[1] + ";encrypt=false;trustServerCertificate=true");
    ...
    private void configureFromSecretsManager(HikariConfig config) {
        try {
            ...
            try (SecretsManagerClient client = SecretsManagerClient.builder()
                ...
                String database = secret.has("dbname") ? secret.get("dbname").asText() : "master";
                String username = secret.get("username").asText();
                String password = secret.get("password").asText();

                config.setJdbcUrl("jdbc:sqlserver://" + host + ":" + port + ";databaseName=" + database + ";encrypt=false;trustServerCertificate=true");
                config.setUsername(username);
                config.setPassword(password);
                config.setDriverClassName("com.microsoft.sqlserver.jdbc.SQLServerDriver");
                
                logger.info("Database configuration completed - JDBC URL: jdbc:sqlserver://{}:{};databaseName={}", host, port, database);
            }
        } catch (Exception e) {
