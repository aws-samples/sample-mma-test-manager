...
public class DatabaseConfig {
    ...
    private void configureFromPassword(HikariConfig config) {
        ...
        String database = hostPortDb.length > 1 ? hostPortDb[1] : "master";
        config.setJdbcUrl("jdbc:sybase:Tds:" + hostPort[0] + ":" + hostPort[1] + "/" + database);
        config.setUsername(userPass[0]);
        config.setPassword(userPass[1]);
        config.setDriverClassName("com.sybase.jdbc42.jdbc.SybDriver");
    }
    ...
    private void configureFromSecretsManager(HikariConfig config) {
        try {
            ...
            try (SecretsManagerClient client = SecretsManagerClient.builder()
                ...
                config.setJdbcUrl("jdbc:sybase:Tds:" + host + ":" + port + "/" + database);
                config.setUsername(username);
                config.setPassword(password);
                config.setDriverClassName("com.sybase.jdbc42.jdbc.SybDriver");
                
                logger.info("Database configuration completed - JDBC URL: jdbc:jtds:sybase://{}:{}/{}", host, port, database);
            }
        } catch (Exception e) {
